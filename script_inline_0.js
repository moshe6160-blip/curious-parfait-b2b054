...
function visibleNotes(row){
  ...
  ["DNAMT","CANCELAMT","CREDIT","CREDITNO","CREDITDATE","CREDITREASON","CREDITLINK","CREDITPARENT","PARENTINVOICE","CLOSE","GLITEMS"].forEach(tag => n = stripNoteTag(n, tag));
  return n;
}
    ...
    <div id="entryModal" class="modal">
      <div class="modal-box">
        ...
        <div class="form-grid">
          ...
          <label id="entryOrderWrap"><span id="entryOrderLabel">Our Order No</span><input class="dark" id="entryOrderNo" type="text" placeholder="PO-001" readonly></label>
          ...
          <input type="hidden" id="entryDescriptionSelect" value="">
          <input type="hidden" id="entryDescription" value="">

          <div id="glItemsSection" class="full gl-items-section">
            <div class="gl-title-row">
              <div><b>Order / Invoice Items</b><div class="helper">Add supplier-style item rows. Every line has its own GL Code and GL Description.</div></div>
              <button type="button" class="primary" onclick="window.vardoGlAddItem && window.vardoGlAddItem()">+ Add Item</button>
            </div>
            <div class="gl-table-wrap">
              <table class="gl-items-table">
                <thead><tr><th>Product Code</th><th>Description</th><th>Qty</th><th>Unit Price</th><th>Discount %</th><th>Net Price</th><th>Total Excl VAT</th><th>GL Code</th><th>GL Description</th><th></th></tr></thead>
            ...
            <div class="gl-actions"><button type="button" class="ghost" onclick="window.vardoGlAccountingSummary && window.vardoGlAccountingSummary()">Generate Accounting</button><button type="button" class="ghost" onclick="window.vardoGlExportCSV && window.vardoGlExportCSV()">Export GL CSV</button><button type="button" class="ghost" onclick="window.vardoPrintCurrentOrder && window.vardoPrintCurrentOrder()">Print Order</button></div>
            <div id="glAccountingSummary" class="gl-summary"></div>
          </div>
...
window.openEntryModal = async function(id=null, forcedMode=""){
  ...
  const editableIds = ["entrySupplier","entryOrderNo","entrySupplierOrderNo","entryInvoiceNo","entryProject","entryType","entryDescription","entryDescriptionSelect","entryNetAmount","entryVatAmount","entryTotal","entryStatus","entryNotes"];
...
/* V279 safe bridge for external UI helpers */
window.__vpGetSelectedIds = function(){ return Array.isArray(selectedIds) ? selectedIds.slice() : []; };
window.__vpDb = supabase;
