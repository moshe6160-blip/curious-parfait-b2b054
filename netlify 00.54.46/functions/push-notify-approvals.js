...
exports.handler = async (event) => {
  try {
    ...
    const total = first && first.total ? String(first.total) : '';
    const detail = [orderNo, supplier, total].filter(Boolean).join(' · ');
    const payload = JSON.stringify({
      title: 'Vardophase: אישור הזמנה נדרש',
      body: count === 1 ? `Pre-Order ממתין לאישור${detail ? ': ' + detail : ''}` : `יש ${count} Pre-Orders שממתינים לאישור`,
    ...
    if (body.subscription && body.subscription.endpoint) {
      targets = [{ subscription: body.subscription, key: null }];
    } else {
      targets = await listSubscriptions();
    }
    ...
    return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ok: sent > 0, sent, targets: targets.length, mode: 'true-push-blobs', results }) };
  } catch (err) {
