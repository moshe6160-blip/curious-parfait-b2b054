(function(){
  'use strict';
  if(window.__v346DepositLogicReportFix) return;
  window.__v346DepositLogicReportFix = true;
  ...
  function summary(rows){
    ...
    rows.forEach(r=>{
      const k=kind(r), a=total(r), n=net(r), v=vat(r), d=due(r);
      if(k==='Deposit'){
        // Deposit is an advance / supplier credit. It must NOT be counted as paid, net, VAT, invoice, or order turnover.
        s.deposits+=a;
        return;
      }
      s.net+=n; s.vat+=v; s.total+=a; s.outstanding+=d;
      if(k==='Invoice') s.invoices+=a; else if(k==='Order') s.orders+=a;
      if(isPaid(r)) s.paid+=a;
      if(d>0) s.unpaid+=d;
    });
    return s;
  }
  ...
  function status(r){
    if(isDeposit(r)) return '<span class="v345-status deposit">Deposit / Advance</span>';
  ...
  function openSupplierReport(names, rows){
    ...
    const supplierCard=one?'<div class="split"><div class="box"><h2 style="margin-top:0">Supplier Card</h2>'+[
      ...
      ['Orders',money(s.orders)],['Invoices',money(s.invoices)],['Deposits / Advances',money(s.deposits)],['Paid',money(s.paid)],['Unpaid',money(s.unpaid)],['Outstanding',money(s.outstanding)],['Entries',s.count]
  ...
  function installSearch(){
    ...
    ['v340GlobalSearch','v344GlobalSearch','v346GlobalSearch'].forEach(id=>{ const el=document.getElementById(id); if(el) el.remove(); });
    if(!document.getElementById('v346SearchStyle')){
      const st=document.createElement('style'); st.id='v346SearchStyle'; st.textContent=`
    ...
    const box=document.createElement('div'); box.id='v346GlobalSearch'; box.className='v345-global-search';
    box.innerHTML='<input id="v346GlobalSearchInput" placeholder="Search supplier / PO / supplier order / GL / description..." autocomplete="off"><div id="v346GlobalSearchResults" class="v345-results"></div>';
    document.body.appendChild(box);
    const input=box.querySelector('input'), out=box.querySelector('#v346GlobalSearchResults'); let timer=null, cache=null;
  ...
  function boot(){
    window.runSupplierReport = v346RunSupplierReport;
    window.v346OpenSupplierReport = openSupplierReport;
    installSearch();
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot); else boot();
  window.addEventListener('load',()=>setTimeout(boot,600));
  // keep last override without touching login/order logic
  setTimeout(()=>{ window.runSupplierReport = v346RunSupplierReport; if(!document.getElementById('v346GlobalSearch')) installSearch(); },1500); document.addEventListener('click',()=>setTimeout(()=>{ window.runSupplierReport = v346RunSupplierReport; if(!document.getElementById('v346GlobalSearch')) installSearch(); },300),true);
})();
