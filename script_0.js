...
(function(){
 ...
 function inject(){ if(!canShow()) return; var bars=Array.from(document.querySelectorAll('.toolbar,.supplier-selection-toolbar,div')); var bar=bars.find(function(el){var t=(el.textContent||'').toLowerCase(); return (t.includes('mark selected')&&t.includes('delete selected'))||(t.includes('convert order')&&t.includes('duplicate selected'));}); if(bar&&!document.getElementById('v264CreditNoteMainBtn')){var b=make('v264CreditNoteMainBtn','Create Credit Note'); bar.insertBefore(b,bar.firstElementChild||null); try{bar.style.gridTemplateColumns='repeat(auto-fit,minmax(145px,1fr))';}catch(e){}} if(!document.getElementById('v264CreditNoteFloatBtn')){var f=make('v264CreditNoteFloatBtn','Credit Note'); f.style.position='fixed'; f.style.right='18px'; f.style.bottom='82px'; f.style.minHeight='46px'; f.style.padding='0 22px'; f.style.borderRadius='999px'; f.style.fontSize='14px'; document.body.appendChild(f);} }
 var st=document.createElement('style'); st.textContent='.v264-credit-note-force{opacity:1!important;visibility:visible!important;pointer-events:auto!important}@media(max-width:700px){#v264CreditNoteFloatBtn{right:12px!important;bottom:92px!important}}'; document.head.appendChild(st);
