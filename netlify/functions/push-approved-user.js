...
exports.handler = async (event) => {
  try {
    ...
    if (!subscription || !subscription.endpoint) {
      return { statusCode: 400, body: 'No subscription supplied' };
    }
    ...
    return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ok: true, sent: 1, mode: 'approved-user-safe' }) };
  } catch (err) {
