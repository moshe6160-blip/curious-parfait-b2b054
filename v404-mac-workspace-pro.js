(function(){
  'use strict';
  const VERSION = 'V404_MAC_WORKSPACE_PRO';
...
/* V404 PRO: Gemini/Mac magic workspace */
#v404DockToggle{position:fixed;right:12px;bottom:calc(18px + env(safe-area-inset-bottom,0px));z-index:2147482700;border:1px solid rgba(246,219,173,.25);background:rgba(10,11,14,.82);color:#f7dcaf;border-radius:999px;padding:8px 12px;font:900 12px -apple-system,BlinkMacSystemFont,Segoe UI,Arial;box-shadow:0 10px 30px rgba(0,0,0,.45);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}
#v403Dock.v404-dock-hidden{transform:translateX(-50%) translateY(calc(100% + 18px));opacity:.08;pointer-events:none}.v404-dock-hidden-body{padding-bottom:12px!important}
.v404-modal-workspace{background:transparent!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;pointer-events:none!important;align-items:initial!important;justify-content:initial!important;overflow:visible!important;padding:0!important;z-index:2147482300!important}.v404-modal-workspace.show{display:block!important}.v404-modal-workspace .modal-box{pointer-events:auto!important;max-height:calc(100vh - 96px);overflow:auto;border-radius:24px!important;box-shadow:0 28px 90px rgba(0,0,0,.68),0 0 0 1px rgba(246,219,173,.20),inset 0 1px 0 rgba(255,255,255,.10)!important;backdrop-filter:blur(28px);-webkit-backdrop-filter:blur(28px)}.v404-modal-box.dragging{opacity:.94;user-select:none}.v404-minimize-entry{border:0;border-radius:999px;background:linear-gradient(135deg,#f7dfb7,#c18a4a);color:#151515;font-weight:900;padding:8px 11px;cursor:pointer;margin-left:auto}.v404-magic-open{animation:v404MagicOpen .34s cubic-bezier(.18,.86,.28,1.12)}@keyframes v404MagicOpen{0%{opacity:0;transform:translateX(-50%) translateY(24px) scale(.965);filter:blur(8px)}100%{opacity:1;filter:blur(0)}}@media(max-width:680px){.v404-magic-open{animation:v404MagicOpenMobile .32s cubic-bezier(.18,.86,.28,1.12)}@keyframes v404MagicOpenMobile{0%{opacity:0;transform:translateY(18px) scale(.97);filter:blur(8px)}100%{opacity:1;filter:blur(0)}}#v404DockToggle{right:8px;bottom:calc(10px + env(safe-area-inset-bottom,0px));padding:7px 10px}.v404-modal-workspace .modal-box{width:calc(100vw - 16px)!important;max-width:calc(100vw - 16px)!important}}
...
`;
  ...
  function openOrders(){ return openEntryWorkspace('order'); }
  function openDN(){ return openEntryWorkspace('delivery_note'); }
  function openInvoice(){ return openEntryWorkspace('invoice'); }
  ...
  // ===== V404 PRO Workspace Upgrade: real modal minimize/restore + free drag + dock hide =====
  const entryDrafts = {};
  let entryDraftSeq = 1;
  let patchedEntryModal = false;

  function getEntryModeLabel(mode){
    const m=String(mode||'').toLowerCase();
    if(m==='delivery_note') return ['🚚','Delivery Note'];
    if(m==='invoice') return ['🧾','Invoice'];
    if(m==='deposit') return ['💰','Deposit'];
    return ['📦','Order'];
  }
  function getCurrentEntryMode(){
    return qs('#entryMode')?.value || qs('#entryType')?.value || 'order';
  }
  function collectEntryForm(){
    const ids=['entrySupplier','entrySupplierVatType','entryOrderNo','entryInvoiceNo','entryProject','entryType','entryDescription','entryDescriptionSelect','entryNetAmount','entryVatAmount','entryTotal','entryStatus','entryNotes','entryMode'];
    const data={}; ids.forEach(id=>{ const el=qs('#'+id); if(el) data[id]=el.value; });
    return data;
  }
  function applyEntryForm(data){
    if(!data) return;
    Object.keys(data).forEach(id=>{ const el=qs('#'+id); if(el) el.value=data[id]??''; });
    try{ window.handleEntryTypeChange?.(); window.handleSupplierVatTypeChange?.(); window.applyEntryModePermissions?.(); }catch(_e){}
  }
  function entryTitleFromForm(mode){
    const [icon,label]=getEntryModeLabel(mode);
    const no=qs('#entryOrderNo')?.value || qs('#entryInvoiceNo')?.value || '';
    return icon+' '+label+(no?' '+no:' Draft');
  }
  function minimizeEntryModal(){
    const modal=qs('#entryModal'); if(!modal || !modal.classList.contains('show')) return;
    const mode=getCurrentEntryMode();
    const id='entryDraft_'+(entryDraftSeq++);
    const title=entryTitleFromForm(mode);
    entryDrafts[id]={ mode, title, data:collectEntryForm(), editingId: (typeof window.editingId!=='undefined'?window.editingId:null), ts:Date.now() };
    modal.classList.remove('show');
    addEntryDraftChip(id,title);
    updateBadges();
    toast('Window minimized to Dock');
  }
  async function restoreEntryDraft(id){
    const d=entryDrafts[id]; if(!d) return;
    if(typeof window.openEntryModal==='function'){
      await window.openEntryModal(null,d.mode);
      setTimeout(()=>{ applyEntryForm(d.data); enhanceEntryModal(d.mode); bringNativeModalFront(); },80);
    }
    const chip=qs('#v403Chip_'+id); if(chip) chip.remove();
    delete entryDrafts[id];
    updateBadges();
  }
  function addEntryDraftChip(id,title){
    const dock=ensureDock();
    if(qs('#v403Chip_'+id)) return;
    const b=dockButton('chip_'+id,'📝',title,()=>restoreEntryDraft(id),'v403-window-chip active');
    b.id='v403Chip_'+id; b.dataset.entryDraft=id; b.draggable=true;
    b.addEventListener('dragstart',e=>{ e.dataTransfer.setData('text/plain', title); e.dataTransfer.setData('text/html','<b>'+esc(title)+'</b>'); });
    dock.appendChild(b);
  }
  function enhanceEntryModal(mode){
    const modal=qs('#entryModal'), box=qs('#entryModal .modal-box'); if(!modal||!box) return;
    modal.classList.add('v404-modal-workspace');
    box.classList.add('v404-modal-box');
    if(!box.dataset.v404Positioned){
      const vw=window.innerWidth;
      box.style.position='fixed'; box.style.left=(vw>720?'50%':'8px'); box.style.top=(vw>720?'64px':'58px'); box.style.transform=(vw>720?'translateX(-50%)':'none');
      box.dataset.v404Positioned='1';
    }
    const head=qs('#entryModal .window-bar') || qs('#entryModal .modal-head');
    if(head && !head.dataset.v404Drag){ makeElementDraggable(box, head); head.dataset.v404Drag='1'; }
    if(!qs('#entryModal .v404-minimize-entry')){
      const btn=document.createElement('button'); btn.type='button'; btn.className='v404-minimize-entry'; btn.textContent='— Dock'; btn.title='Minimize this document to Dock'; btn.onclick=minimizeEntryModal;
      const mh=qs('#entryModal .modal-head'); if(mh) mh.appendChild(btn); else box.prepend(btn);
    }
    const title=qs('#entryModalTitle'); if(title && !title.dataset.v404Magic){ title.dataset.v404Magic='1'; title.style.cursor='grab'; }
    box.classList.remove('v404-magic-open'); void box.offsetWidth; box.classList.add('v404-magic-open');
  }
  function bringNativeModalFront(){ const box=qs('#entryModal .modal-box'); if(box) box.style.zIndex=2147482450; }
  function makeElementDraggable(box, handle){
    let dragging=false,sx=0,sy=0,ox=0,oy=0;
    handle.addEventListener('pointerdown',e=>{ if(e.target.closest('button,input,select,textarea')) return; dragging=true; box.classList.add('dragging'); sx=e.clientX; sy=e.clientY; const r=box.getBoundingClientRect(); ox=r.left; oy=r.top; box.style.transform='none'; handle.setPointerCapture?.(e.pointerId); });
    handle.addEventListener('pointermove',e=>{ if(!dragging) return; const nx=ox+(e.clientX-sx); const ny=oy+(e.clientY-sy); box.style.left=Math.max(4,Math.min(window.innerWidth-70,nx))+'px'; box.style.top=Math.max(4,Math.min(window.innerHeight-80,ny))+'px'; });
    handle.addEventListener('pointerup',e=>{ dragging=false; box.classList.remove('dragging'); try{handle.releasePointerCapture(e.pointerId)}catch(_e){} });
  }
  function toggleDockHidden(){
    const dock=ensureDock();
    const hidden=dock.classList.toggle('v404-dock-hidden');
    document.body.classList.toggle('v404-dock-hidden-body',hidden);
    localStorage.setItem('v404_dock_hidden', hidden?'1':'0');
    const t=qs('#v404DockToggle'); if(t) t.textContent=hidden?'⌃ Dock':'⌄ Dock';
  }
  function ensureDockToggle(){
    if(qs('#v404DockToggle')) return;
    const b=document.createElement('button'); b.id='v404DockToggle'; b.type='button'; b.textContent='⌄ Dock'; b.onclick=toggleDockHidden; document.body.appendChild(b);
    setTimeout(()=>{ if(localStorage.getItem('v404_dock_hidden')==='1') toggleDockHidden(); },250);
  }
  async function openEntryWorkspace(mode){
    if(typeof window.openEntryModal==='function'){
      await window.openEntryModal(null,mode);
      setTimeout(()=>enhanceEntryModal(mode),60);
    } else toast('Entry modal not found');
  }
  function patchEntryModalOpen(){
    if(patchedEntryModal || typeof window.openEntryModal!=='function') return;
    patchedEntryModal=true;
    ...
    window.openEntryModal=async function(id=null, forcedMode=''){
      ...
      setTimeout(()=>enhanceEntryModal(forcedMode||getCurrentEntryMode()),80);
      return res;
    };
  }
  ...
  window.v403UpdateDockBadges=updateBadges; window.v404ToggleDockHidden=toggleDockHidden; window.v404MinimizeEntryModal=minimizeEntryModal;
  function boot(){ ensureDock(); ensureDockToggle(); patchEntryModalOpen(); console.log(VERSION,'loaded'); }
