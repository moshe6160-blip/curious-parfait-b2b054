(function(){
  ...
  function normalizeContractor(c){
    ...
    return {
      ...
      paymentTerms: c.paymentTerms || c.payment_terms || c.terms || '',
  ...
  function cards(data){
    ...
    return data.map(c=>{ const t=totals(c); const alert=t.overOriginalContract>0?`<div class="line"><span class="red">Above original contract</span><b class="overColor">${money(t.overOriginalContract)} · ${t.progress.toFixed(1)}%</b></div>`:''; return `<div class="card ${c.id===activeId?'active':''}" data-vpcard="${esc((c.name+' '+c.trade+' '+c.project+' '+(c.budgetItem||'')+' '+(c.paymentTerms||'')).toLowerCase())}" onclick="vpConSelect('${c.id}')"><h3 style="margin:0 0 8px!important">${esc(c.name)}</h3><div class="line"><span>${esc(c.trade||'Sub Contractor')}</span><b class="contractColor">${money(c.contract)}</b></div><div class="line"><span>Budget Item</span><b class="retentionColor">${esc(c.budgetItem||'-')}</b></div><div class="line"><span>Budget</span><b class="retentionColor">${money(c.budget || t.approvedBudget)}</b></div><div class="line"><span>Claimed</span><b class="claimedColor">${money(t.claimed)}</b></div><div class="line"><span>Paid</span><b class="paidColor">${money(t.paid)}</b></div><div class="line"><span>Outstanding</span><b class="outstandingColor">${money(t.outstanding)}</b></div>${alert}<div class="progress"><i style="width:${Math.min(100,t.budgetProgress)}%"></i></div></div>`; }).join('');
  }
  ...
  function detail(c){
    ...
    return `<div class="row"><div><h1 style="font-size:24px!important">${esc(c.name)}</h1><div class="sub">${esc(c.trade||'')} • ${esc(c.project||'')}${c.budgetItem ? ' • '+esc(c.budgetItem) : ''}${c.paymentTerms ? ' • Terms: '+esc(c.paymentTerms) : ''}</div></div><div class="row" style="justify-content:flex-start!important"><button class="btn" onclick="vpConPrint()">Generate Statement / PDF</button><button class="btn danger" onclick="vpConDeleteContractor('${c.id}')">Delete Contractor</button></div></div>
  ...
  window.vpConSaveContractorSetup = id => updateContractor(id, c => {
    ...
    c.budget = num(q('contractorBudgetEdit')?.value);
    c.paymentTerms = (q('contractorPaymentTermsEdit')?.value || '').trim();
    c.retention = Number(q('contractorRetentionEdit')?.value || c.retention || 5);
    c.notes = (q('contractorNotesEdit')?.value || '').trim();
  });
