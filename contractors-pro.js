(function(){
  ...
  function detail(c){
    ...
    return `<div class="row"><div><h1 style="font-size:24px!important">${esc(c.name)}</h1><div class="sub">${esc(c.trade||'')} • ${esc(c.project||'')}</div></div><div class="row" style="justify-content:flex-start!important"><button class="btn" onclick="vpConPrint()">Generate Statement / PDF</button><button class="btn danger" onclick="vpConDeleteContractor('${c.id}')">Delete Contractor</button></div></div>
      <div class="grid6"><div class="mini"><span>Original Contract</span><b>${money(c.contract)}</b></div><div class="mini"><span>Variations / Additions</span><b>${money(t.variationsTotal)}</b></div><div class="mini"><span>Approved Budget</span><b>${money(t.approvedBudget)}</b></div><div class="mini"><span>Claimed</span><b>${money(t.claimed)}</b></div><div class="mini"><span>Paid Total</span><b>${money(t.paid)}</b></div><div class="mini"><span>Outstanding</span><b class="${t.outstanding>0?'red':'green'}">${money(t.outstanding)}</b></div></div>
      <div class="grid4"><div class="mini"><span>Progress vs Contract</span><b class="${t.progress>100?'red':'green'}">${t.progress.toFixed(1)}%</b></div><div class="mini"><span>Progress vs Budget</span><b>${t.budgetProgress.toFixed(1)}%</b></div><div class="mini"><span>Above Original Contract</span><b class="${t.overOriginalContract>0?'red':'green'}">${money(t.overOriginalContract)}</b></div><div class="mini"><span>Available For Accounts</span><b>${money(t.availableForAccounts)}</b></div></div>
      <div class="grid4"><div class="mini"><span>Paid Accounts</span><b>${t.paidAccounts} / ${t.totalAccounts}</b></div><div class="mini"><span>Open Accounts</span><b class="red">${t.openAccounts}</b></div><div class="mini"><span>Retention Held</span><b>${money(t.retentionHeld)}</b></div><div class="mini"><span>Retention Balance</span><b>${money(t.retentionBalance)}</b></div></div>
