...
/* V333 order items UI + in-process/final workflow fix.
   Fixes V330 missing item-table functions: PO numbering worked, but items could not be collected/saved/loaded. */
(function(){
  if(window.__v333OrderItemsWorkflowInstalled) return;
  window.__v333OrderItemsWorkflowInstalled = true;
  ...
  function isOrderMode(){
    ...
    return mode === 'order' || (!!orderNo && !norm(document.getElementById('entryInvoiceNo')?.value));
  }

  function ensureStatusOptions(){
    const st = document.getElementById('entryStatus');
    if(!st) return;
    ['Order in Process','Final Order','Paid','Unpaid'].forEach(v=>{
      if(!Array.from(st.options || []).some(o=>o.value===v)){
        const opt=document.createElement('option'); opt.value=v; opt.textContent=v; st.appendChild(opt);
      }
    });
    if(isOrderMode() && (!st.value || st.value === 'Paid' || st.value === 'Unpaid')) st.value = 'Order in Process';
  }

  function itemRowHtml(it={}){
    const code = norm(it.productCode || it.product_code || '');
    const desc = norm(it.description || '');
    const qty = num(it.qty || 0) || '';
    const price = num(it.unitPrice ?? it.unit_price ?? it.price ?? 0) || '';
    const disc = num(it.discountPercent ?? it.discount ?? 0) || '';
    const gl = norm(it.glCode || it.gl_code || '');
    const gld = norm(it.glDescription || it.gl_description || '');
    return `<tr>
      <td><input class="dark gli-code" value="${code.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('"','&quot;')}" placeholder="Code"></td>
      <td><input class="dark gli-desc" value="${desc.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('"','&quot;')}" placeholder="Description / item"></td>
      <td><input class="dark gli-qty" type="number" step="0.01" value="${qty}" placeholder="Qty" oninput="window.vardoGlRecalcItems()"></td>
      <td><input class="dark gli-price" type="number" step="0.01" value="${price}" placeholder="Unit price" oninput="window.vardoGlRecalcItems()"></td>
      <td><input class="dark gli-disc" type="number" step="0.01" value="${disc}" placeholder="Disc %" oninput="window.vardoGlRecalcItems()"></td>
      <td><input class="dark gli-gl" value="${gl.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('"','&quot;')}" placeholder="GL code"></td>
      <td><input class="dark gli-gldesc" value="${gld.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('"','&quot;')}" placeholder="GL description"></td>
      <td class="gli-total">0.00</td>
      <td><button type="button" class="red" onclick="this.closest('tr').remove(); window.vardoGlRecalcItems();">Remove</button></td>
    </tr>`;
  }

  window.vardoGlAddItem = function(item={}){
    ensureOrderItemsUi();
    const body = document.getElementById('glItemsBody');
    if(!body) return;
  ...
  window.vardoGlCollectItems = function(){
    const rows = Array.from(document.querySelectorAll('#glItemsBody tr'));
    return rows.map(tr=>{
      const qty = num(tr.querySelector('.gli-qty')?.value);
      const unitPrice = num(tr.querySelector('.gli-price')?.value);
      const discountPercent = num(tr.querySelector('.gli-disc')?.value);
      const netPrice = unitPrice * (1 - (discountPercent/100));
      const totalExVat = qty * netPrice;
      return {
        productCode: norm(tr.querySelector('.gli-code')?.value),
        description: norm(tr.querySelector('.gli-desc')?.value),
        qty, unitPrice, discountPercent,
        glCode: norm(tr.querySelector('.gli-gl')?.value),
        glDescription: norm(tr.querySelector('.gli-gldesc')?.value),
        totalExVat
      };
    }).filter(it => it.productCode || it.description || it.qty || it.unitPrice || it.discountPercent || it.glCode || it.glDescription);
  };

  window.vardoGlRecalcItems = function(){
    let subtotal = 0;
    Array.from(document.querySelectorAll('#glItemsBody tr')).forEach(tr=>{
      ...
      subtotal += total;
      const cell = tr.querySelector('.gli-total'); if(cell) cell.textContent = money2(total);
    });
    const sub = document.getElementById('glItemsSubtotal'); if(sub) sub.textContent = money2(subtotal);
    const vat = subtotal > 0 ? subtotal * 0.15 : 0;
    const total = subtotal + vat;
    const netEl = document.getElementById('entryNetAmount');
    const vatEl = document.getElementById('entryVatAmount');
    const totalEl = document.getElementById('entryTotal');
    if(isOrderMode() && (document.querySelectorAll('#glItemsBody tr').length > 0)){
      if(netEl) netEl.value = money2(subtotal);
      if(vatEl) vatEl.value = money2(vat);
      if(totalEl) totalEl.value = money2(total);
    }
  };

  function ensureOrderItemsUi(){
    const modal = document.getElementById('entryModal');
    const form = modal?.querySelector('.form-grid');
    if(!modal || !form) return;
    ensureStatusOptions();
    if(!document.getElementById('entrySupplierOrderNo')){
      const orderWrap = document.getElementById('entryOrderWrap');
      if(orderWrap){
        orderWrap.insertAdjacentHTML('afterend', `<label id="supplierOrderWrap"><span>Supplier Quote / Code</span><input class="dark" id="entrySupplierOrderNo" type="text" placeholder="Supplier code / quote no"></label>`);
      }
    }
    if(document.getElementById('orderItemsSection')) return;
    const notesLabel = document.getElementById('entryNotes')?.closest('label');
    const html = `<div id="orderItemsSection" class="full" style="grid-column:1/-1;margin-top:8px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-bottom:8px">
        <div><strong>Order Items</strong><div class="helper">Items stay saved while the order is in process. Use Final Order only when the order is closed.</div></div>
        <button type="button" class="soft" onclick="window.vardoGlAddItem({})">+ Add Item</button>
      </div>
      <div class="table-wrap" style="max-height:none;overflow:auto">
        <table class="gl-items-table">
          <thead><tr><th>Code</th><th>Description</th><th>Qty</th><th>Unit Price</th><th>Disc %</th><th>GL Code</th><th>GL Description</th><th>Total</th><th></th></tr></thead>
          <tbody id="glItemsBody"></tbody>
        </table>
      </div>
      <div class="helper" style="margin-top:8px">Items subtotal: <strong id="glItemsSubtotal">0.00</strong></div>
    </div>`;
    if(notesLabel) notesLabel.insertAdjacentHTML('beforebegin', html);
    else form.insertAdjacentHTML('beforeend', html);
    if(!document.querySelector('#glItemsBody tr')) window.vardoGlAddItem({});
  }
  window.ensureOrderItemsUi = ensureOrderItemsUi;

  window.saveOrderInProcess = async function(){
    ensureOrderItemsUi();
    const st = document.getElementById('entryStatus'); if(st) st.value = 'Order in Process';
    const mode = document.getElementById('entryMode'); if(mode) mode.value = 'order';
    return window.saveEntry();
  };
  window.finalizeOrder = async function(){
    ensureOrderItemsUi();
    const st = document.getElementById('entryStatus'); if(st) st.value = 'Final Order';
    const mode = document.getElementById('entryMode'); if(mode) mode.value = 'order';
    return window.saveEntry();
  };
  ...
  function ensureWorkflowButtons(){
    ...
    if(!actions || document.getElementById('saveProcessBtn')) return;
    const primary = actions.querySelector('button.primary');
    if(primary) primary.textContent = 'Save';
    const wrap = document.createElement('div');
    wrap.style.display='flex'; wrap.style.gap='8px'; wrap.style.flexWrap='wrap';
    wrap.innerHTML = `<button id="saveProcessBtn" type="button" class="soft" onclick="window.saveOrderInProcess()">Save In Process</button><button id="finalizeOrderBtn" type="button" class="primary" onclick="window.finalizeOrder()">Finalize Order</button>`;
  ...
  if(typeof prevOpen === 'function'){
    window.openEntryModal = async function(){
      ...
      const run = ()=>{ ensureOrderItemsUi(); ensureWorkflowButtons(); ensureStatusOptions(); window.vardoGlRecalcItems(); };
      run(); setTimeout(run, 200); setTimeout(run, 800); setTimeout(run, 1500);
      return r;
    };
  }
  ...
  if(typeof prevRender === 'function'){
    window.render = async function(){
      ...
      setTimeout(()=>{ if(document.getElementById('entryModal')?.classList.contains('show')){ ensureOrderItemsUi(); ensureWorkflowButtons(); ensureStatusOptions(); } }, 50);
      return r;
    };
  }
  ...
  try{
    if(typeof processStatusLabel === 'function'){
      const oldStatus = processStatusLabel;
      processStatusLabel = function(row){
        const status = norm(row?.status).toLowerCase();
        const hasOrder = !!norm(row?.order_no);
        const hasDN = (typeof extractDeliveryNoteNo === 'function') ? !!extractDeliveryNoteNo(row) : false;
        const hasInvoice = !!norm(row?.invoice_no);
        if(hasOrder && !hasDN && !hasInvoice){
          if(status === 'final order' || status === 'closed' || status === 'order') return 'Final Order';
          return 'Order in Process';
        }
        return oldStatus(row);
      };
    }
  }catch(e){}
})();
