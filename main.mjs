...
/* V275: clear invoice credit statuses only (UI/status display, login untouched) */
function invoiceCreditStatusInfo(row){
  try{
    if(typeof displayEntryKind === "function" && displayEntryKind(row) === "credit_note"){
      return { label: "Credit Note", cls: "credit-note-status" };
    }
    const hasInvoice = !!String(row?.invoice_no || "").trim();
    if(!hasInvoice) return null;
    const credit = (typeof extractCreditAmount === "function") ? Math.abs(Number(extractCreditAmount(row) || 0)) : 0;
    if(credit <= 0.01) return null;
    const invoiceValue = (typeof invoiceDisplayAmount === "function") ? Math.abs(Number(invoiceDisplayAmount(row) || 0)) : Math.abs(Number(row?.total || row?.amount || 0));
    if(invoiceValue > 0 && credit >= invoiceValue - 0.01){
      return { label: "Credited (full)", cls: "credited-full" };
    }
    return { label: "Credited (partial)", cls: "credited-partial" };
  }catch(e){ return null; }
}
function displayFinancialStatusLabel(row){
  const info = invoiceCreditStatusInfo(row);
  return info ? info.label : String(row?.status || "");
}
function displayFinancialStatusClass(row){
  const info = invoiceCreditStatusInfo(row);
  if(info) return info.cls;
  const st = String(row?.status || "").toLowerCase();
  if(st.includes("paid") || st.includes("covered")) return "paid";
  if(st.includes("partial") || st.includes("credit")) return "credited-partial";
  return "unpaid";
}
            ...
            <tbody>
              ${entries.map(e=>`
                <tr class="${selectedIds.includes(e.id)?'selected':''} ${displayEntryKind(e)==="order" && daysOld(e.created_at)>=60 ? 'order-60' : (displayEntryKind(e)==="order" && daysOld(e.created_at)>=30 ? 'order-30' : '')}" onclick="if(event.target.closest('button') || event.target.closest('input') || event.target.closest('select') || event.target.closest('label')) return; window.openEntryModal('${e.id}')">
                  ...
                  <td><span class="badge ${typeof displayFinancialStatusClass === "function" ? displayFinancialStatusClass(e) : (e.status==='Paid'?'paid':'unpaid')}">${esc(typeof displayFinancialStatusLabel === "function" ? displayFinancialStatusLabel(e) : (e.status || ""))}</span></td>
...
(function(){
  ...
  window.openCreditNoteModal = async function(){
    try{
      ...
      const safeStatus = invoiceDueAfterCredit <= 0.01 ? "Credited (full)" : "Credited (partial)";
...
(function(){
  ...
  function build(){
    try{
      ...
      // Final credit logic:
      // Credit Balance is ONLY unused supplier deposits / advance payments.
      // Invoice-linked Credit Notes appear as negative Deposit Applied, but they must NOT
      // become supplier Credit Balance. Example: Invoice 12,000 + Credit Note -6,000
      // should show net Invoiced 6,000 and Credit Balance 0.
      const positiveDepositApplied = Math.max(0, totals.depositApplied);
      totals.creditBalance = Math.max(0, totals.depositCredit - positiveDepositApplied);
...
<script id="v88-login-click-guard">
...
<script id="v96-daily-dn-invoice-final-fix">
(function(){
  function localKey(d){
...
<script id="v97-local-date-final-fix">
(function(){
...
<script src="contractors-pro.js" defer></script>
<script>
...
<script id="v277-accounting-true-net-from-v276-main">
(function(){
...
window.reverseDN = async function(){
  try{
    ...
    if(typeof selectedIds === "undefined") return alert("Selection is not ready. Refresh and try again.");
    ...
    if(String(data.invoice_no || "").trim()) return alert("Cannot reverse DN after Invoice exists. Reverse/cancel the invoice first.");
    if(!confirm("Reverse DN " + dnNo + " and return this row to Order?")) return;

    let notes = String(data.notes || "");
    if(typeof stripDeliveryNoteTags === "function") notes = stripDeliveryNoteTags(notes); else notes = notes.replace(/\n?\[\[DN:[^\]]+\]\]/g, "").trim();
    ["DNAMT","CLOSE","CANCELAMT","CREDIT","CREDITNO","CREDITDATE","CREDITREASON","CREDITLINK","CREDITPARENT"].forEach(function(tag){
      if(typeof stripNoteTag === "function") notes = stripNoteTag(notes, tag); else notes = notes.replace(new RegExp("\\n?\\[\\[" + tag + ":[^\\]]*\\]\\]", "g"), "").trim();
    });

    const { error: updateError } = await supabase.from("suppliers").update({ notes:notes, status:"Order", entry_type:"order", invoice_no:"" }).eq("id", data.id);
    ...
    if(typeof render === "function") await render();
  }catch(err){ alert(err?.message || "Reverse DN error"); }
};
...
window.injectReverseDNButton = function(){
  try{
    document.querySelectorAll('.supplier-selection-toolbar').forEach(function(bar){
      ...
      btn.type='button'; btn.className='reverse-dn-btn'; btn.setAttribute('data-v468-reverse-dn','1'); btn.setAttribute('onclick','window.reverseDN()'); btn.textContent='Reverse DN';
      if(del) bar.insertBefore(btn, del); else bar.appendChild(btn);
    });
  }catch(e){}
};
(function(){
  if(!document.getElementById('v468ReverseDNStyle')){
    var st=document.createElement('style'); st.id='v468ReverseDNStyle';
    st.textContent='#app button.reverse-dn-btn,button.reverse-dn-btn{background:linear-gradient(135deg,#d6e0ea 0%,#91a7bb 48%,#5f7488 100%)!important;background-image:linear-gradient(135deg,#d6e0ea 0%,#91a7bb 48%,#5f7488 100%)!important;color:#071018!important;-webkit-text-fill-color:#071018!important;border:1px solid rgba(210,225,238,.78)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.55),0 10px 24px rgba(0,0,0,.30)!important;font-weight:850!important;border-radius:15px!important;min-height:48px!important}#app button.reverse-dn-btn:hover,button.reverse-dn-btn:hover{filter:brightness(1.06)!important}';
    document.head.appendChild(st);
  }
  const oldRender=window.render;
  if(typeof oldRender==='function' && !oldRender.__v468ReverseDN){
    const wrapped=async function(){ const res=await oldRender.apply(this,arguments); setTimeout(window.injectReverseDNButton,0); return res; };
    wrapped.__v468ReverseDN=true; window.render=wrapped;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',function(){setTimeout(window.injectReverseDNButton,0);}); else setTimeout(window.injectReverseDNButton,0);
})();
