...
(function(){
  ...
  function defaultItem(){ return { item:'', itemNo:'', glCode:'', description:'', manualGL:'', manualDescription:'', qty:1, price:'', discount:'', total:0 }; }
  function getVatRate(){
  ...
  function lineSubtotal(x){ return num(x.qty) * num(x.price); }
  function lineDiscountAmount(x){
    const pct = Math.max(0, Math.min(100, num(x.discount)));
    return lineSubtotal(x) * (pct / 100);
  }
  function lineTotal(x){ return Math.max(0, lineSubtotal(x) - lineDiscountAmount(x)); }
  function calcTotals(items){
    let net = 0;
    (items || []).forEach(x => {
      const line = lineTotal(x);
      x.discountAmount = Math.round(lineDiscountAmount(x) * 100) / 100;
  ...
  function currentItems(){
    return Array.from(document.querySelectorAll('#v309OrderItemsBody tr')).map(tr => ({
      item: String(Array.from(tr.parentNode.children).indexOf(tr) + 1),
      itemNo: String(Array.from(tr.parentNode.children).indexOf(tr) + 1),
  ...
  function renderRows(items){
    ...
    body.innerHTML = arr.map((x,i) => {
      const total = lineTotal(x);
      const desc = x.description || glDescription(x.glCode);
      return '<tr>'+
        '<td><input class="dark" data-field="item" value="'+esc(String(i+1))+'" readonly title="Auto item number"></td>'+ 
        ...
        '<td><input class="dark" data-field="discount" type="number" min="0" max="100" step="0.01" value="'+esc(x.discount)+'" placeholder="%" title="Discount percent"><div class="v334-discount-amount" data-field="discountAmount">-'+money(lineDiscountAmount(x))+'</div></td>'+
  ...
  function recalcVisibleTotals(){
    document.querySelectorAll('#v309OrderItemsBody tr').forEach(tr => {
      ...
      if(totalEl) totalEl.value = money(lineTotal({qty,price,discount}));
      const discEl = tr.querySelector('[data-field="discountAmount"]');
      if(discEl) discEl.textContent = '-' + money(lineDiscountAmount({qty,price,discount}));
    });
  ...
  function mountTable(){
    ...
    box.innerHTML = '<div class="v309-title-row"><span>Order Items / GL Allocation</span><button type="button" class="primary" id="v309AddItemRow">+ Add Item</button></div>'+
      ...
      '<div class="v309-scroll"><table class="v309-items-table"><thead><tr><th>#</th><th>Manual Description</th><th>Select GL Code</th><th>Description</th><th>Code</th><th>Qty</th><th>Price</th><th>Discount %</th><th>Total</th><th></th></tr></thead><tbody id="v309OrderItemsBody"></tbody></table></div>'+
      '<div class="helper">Select GL Code to autofill Description and Code. Discount is percentage (%). Manual Description stays editable. Qty can be saved without price.</div>';
  ...
  function injectStyle(){
    ...
    st.textContent = '.v309-order-items-box{grid-column:1/-1;margin:6px 0 10px 0}.v309-title-row{display:flex;justify-content:space-between;align-items:center;margin:8px 0 10px;font-weight:700}.v309-title-row .primary{min-width:110px}.v309-scroll{overflow-x:auto;overflow-y:hidden;border:1px solid rgba(218,178,122,.22);border-radius:18px;background:rgba(0,0,0,.16);max-width:100%}.v309-items-table{min-width:1180px;width:100%;border-collapse:separate;border-spacing:0;table-layout:fixed}.v309-items-table th{font-size:12px;text-align:left;padding:9px;color:#d9b27a;white-space:nowrap}.v309-items-table td{padding:5px}.v309-items-table input,.v309-items-table select{min-height:38px;font-size:13px;width:100%;box-sizing:border-box}.v309-items-table button{min-height:38px;padding:0 12px}.v309-items-table th:nth-child(1),.v309-items-table td:nth-child(1){width:48px;text-align:center}.v309-items-table th:nth-child(2),.v309-items-table td:nth-child(2){width:190px}.v309-items-table th:nth-child(3),.v309-items-table td:nth-child(3){width:220px}.v309-items-table th:nth-child(4),.v309-items-table td:nth-child(4){width:190px}.v309-items-table th:nth-child(5),.v309-items-table td:nth-child(5){width:150px}.v309-items-table th:nth-child(6),.v309-items-table td:nth-child(6),.v309-items-table th:nth-child(7),.v309-items-table td:nth-child(7),.v309-items-table th:nth-child(8),.v309-items-table td:nth-child(8),.v309-items-table th:nth-child(9),.v309-items-table td:nth-child(9){width:95px}.v309-items-table th:last-child,.v309-items-table td:last-child{width:48px}.v309-items-table [data-field="item"]{text-align:center!important;font-weight:800;color:#d9b27a!important;background:rgba(218,178,122,.06)!important}.v309-items-table [data-field="codeDisplay"],.v309-items-table [data-field="total"]{opacity:.9}'+
      ...
      '.v309-items-table [data-field="qty"],.v309-items-table [data-field="price"],.v309-items-table [data-field="discount"],.v309-items-table [data-field="total"]{font-weight:700;text-align:center!important}.v334-discount-amount{font-size:10px;opacity:.72;text-align:center;margin-top:3px;color:#d9b27a}'+
